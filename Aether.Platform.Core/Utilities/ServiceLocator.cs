...
namespace Aether.Platform.Core.Utilities
{
    ...
    /// 使用方式：
    /// <list type="number">
    /// <item>AppBootstrap 启动时通过 <see cref="SetProvider"/> 注入容器。</item>
    /// <item>任意位置通过 <see cref="GetService{T}"/> / <see cref="TryGetService{T}"/> 获取服务。</item>
    /// <item>运行时可通过 <see cref="RegisterSingleton{T}"/> 动态追加注册（延迟初始化场景）。</item>
    /// </list>
    /// </remarks>
    public static class ServiceLocator
    {
        ...
        /// <summary>供运行时动态追加注册使用的内部 ServiceCollection</summary>
        ...
        /// <param name="collection">用于动态追加注册的 ServiceCollection（可选）</param>
        ...
        /// <summary>
        /// 获取已注册的服务实例。
        /// 未注册时抛出 InvalidOperationException。
        /// </summary>
        /// <typeparam name="T">服务类型</typeparam>
